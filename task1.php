<?php

/**
 * Класс для работы с API
 *
 * @author        Torbogoshev Artur
 * @version        v.1.0 (08.04.2022)
 */

class Api
{
    public function __construct()
    {

    }

    /**
     * Заполняет строковый шаблон template данными из объекта object
     *
     * @param array $array
     * @param string $template
     * @return        string
     * @version        v.1.0 (08.04.2022)
     * @author        Torbogoshev Artur
     */
    public function get_api_path(array $array, string $template): string
    {
        //Определили по шаблону соответсвия, вызываем callback что нашли, делаем замену
        return preg_replace_callback('#%(.*)%#isU', function ($matches) use($array) {
            return $array[$matches[1]];
        }, $template);
    }
}

$user =
    [
        'id' => 20,
        'name' => 'John Dow',
        'role' => 'QA',
        'salary' => 100
    ];

$api_path_templates =
    [
        "/api/items/%id%/%name%",
        "/api/items/%id%/%role%",
        "/api/items/%id%/%salary%"
    ];

$api = new Api();

$api_paths = array_map(function ($api_path_template) use ($api, $user) {
    return $api->get_api_path($user, $api_path_template);
}, $api_path_templates);

echo json_encode($api_paths, JSON_FORCE_OBJECT | JSON_UNESCAPED_UNICODE);

$expected_result = ['/api/items/20/John%20Dow', '/api/items/20/QA', '/api/items/20/100'];